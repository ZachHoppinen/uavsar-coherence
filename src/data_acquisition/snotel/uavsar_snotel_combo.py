...
outfp = Path('/Users/rdcrlzh1/Documents/uavsar-coherence/data/snotel').joinpath('snotel_uavsar_diffs.csv')
...
for stem, ds in dss.items():
    ...
    for t1, t2, pol, heading in tqdm(product(ds.time1.values, ds.time2.values, ds.pol.values, ds.heading.values)):
        ...
        for snotel_id, snotel_data in loc_snotels.items():
            ...
            diffs.loc[(stem, t1, t2, snotel_id), 'inc'] = img['inc'].sel(x = slice(long - tol_50m, long + tol_50m), y = slice(lat + tol_50m, lat - tol_50m)).values.mean()
            diffs.loc[(stem, t1, t2, snotel_id), 'tree_perc'] = img['tree_perc'].sel(x = slice(long - tol_50m, long + tol_50m), y = slice(lat + tol_50m, lat - tol_50m)).values.mean()
            diffs.loc[(stem, t1, t2, snotel_id), 'land_cover'] = img['land_cover'].sel(x = long, y = lat, method = 'nearest').values
            diffs.loc[(stem, t1, t2, snotel_id), 'n'] = int(cor.sel(x = slice(long - tol_50m, long + tol_50m), y = slice(lat + tol_50m, lat - tol_50m)).count().values)
            ...
            for col in ['swe', 'sd', 'temp']:
                diffs.loc[(stem, t1, t2, snotel_id), f'{col}_diff'] = np.round(rows.iloc[1][col] - rows.iloc[0][col], 4)
                diffs.loc[(stem, t1, t2, snotel_id), f'{col}_t1'] = np.round(rows.iloc[0][col], 4)
                diffs.loc[(stem, t1, t2, snotel_id), f'{col}_t2'] = np.round(rows.iloc[1][col], 4)
